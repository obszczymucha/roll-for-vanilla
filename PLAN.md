...
## 2. BLOCKING: one question for the user before Phase 8

**Where does the SR+ number come from?** No export format emits `sr_plus`; SR-PLUS §7
proves the field has never existed in any real payload or fixture. Nothing about SR+ can be
built until this is answered. The options, from SR-PLUS §10.4:

1. **softres.it's existing `rollBonus`** -- real data today, but per *player*, not per item.
2. **Per-item, from a format that emits it** -- what the original code expected; needs a
   source that produces it.
3. **Locally assigned**, e.g. `/sr+ <player> <n>` with its own db -- provider-independent,
   and the only option that works with raidres.top as it exists.

**This choice has an architectural consequence the docs do not spell out.** Options 1 and 2
put the number in provider data, which the library's transformer would then have to carry
through as an opaque per-roller payload -- otherwise a separate `RollForSrPlus` extension
cannot see it, because the store holds transformed rollers, not raw JSON. Option 3 avoids
that entirely. **If the answer is 1 or 2, add a step to Phase 4 for the passthrough.**

Everything in Phases 3-7 proceeds without this answer.
...
3. Write `RollForSoftRes.lua` from `RollForSoftResIt.lua`, minus the migration block
   (Phase 3 rewrites it) and minus the identity strings. It registers **one** RollFor
...
The library is the extension, so `ctx.db( "softres" )` now resolves to
`extension_softres_softres`. Existing users' data is elsewhere. Full reasoning: SR-DIFF §8.5.

1. Copy `extension_softres_it_softres` → `extension_softres_softres` and
   `extension_softres_it_name_matcher` → `extension_softres_name_matcher`. **Copy, do not
   move** -- a user who downgrades must find their list where they left it. Model it on
   `RollForSoftResIt.migrate_from_core` (deep copy, empty-destination-only, guarded by a
   flag that records that it ran even when there was nothing to copy).
2. Keep `RollForSoftResIt`'s existing core→extension migration alive for anyone upgrading
   from a pre-extension RollFor. The oldest installs make two hops.
3. Backfill the provider id for a migrated list: it came from softres.it.

**Done when:** a migration suite proves the two-hop path, the empty-destination guard, and
that the originals survive.
...
5. **If §2 was answered 1 or 2:** add the opaque per-roller passthrough to the library's
   `SoftResDataTransformer` so a modification can read provider-supplied values.
...
6. Preview path for `delta`-style modifiers, consumed by
   `SoftResRollingLogic.format_name_with_rolls` and `DroppedLootAnnounce.print_player`.
   Re-check the `m.split_message` byte budget -- a longer annotation splits the roll call
   earlier (SR-PLUS §9.1).
...
### Phase 8. SR+ itself — **needs §2 answered**

Per SR-PLUS §10.5, SR+ is its own addon, `RollForSrPlus`: a chain link on the read path and
a modifier on the roll path.

1. Source the number per the §2 answer.
...
3. Resolve the first-entry-wins bug rather than reproducing it (SR-PLUS §6.2, reproduced).
...
5. A fixture that actually contains the field, in whatever format §2 settled on.
...
- [ ] Existing users' soft-res lists and manual name matches survive the upgrade.
...
- [ ] SR-DIFF.md and SR-PLUS.md updated where the build diverged from what they predicted.
