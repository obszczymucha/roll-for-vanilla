...
SelectionTreeAllCheckedSpec = {}

function SelectionTreeAllCheckedSpec:should_be_true_when_dungeon_and_every_descendant_are_checked()
  local d = branch( true, { branch( true, { leaf( true ), leaf( true ) } ) } )
  eq( SelectionTree.all_checked( d ), true )
end

function SelectionTreeAllCheckedSpec:should_be_false_when_the_dungeon_itself_is_unchecked()
  local d = branch( false, { branch( true, { leaf( true ) } ) } )
  eq( SelectionTree.all_checked( d ), false )
end

function SelectionTreeAllCheckedSpec:should_be_false_when_a_boss_is_unchecked_even_if_its_items_are_checked()
  local d = branch( true, { branch( false, { leaf( true ) } ) } )
  eq( SelectionTree.all_checked( d ), false )
end

function SelectionTreeAllCheckedSpec:should_be_false_when_a_single_item_is_unchecked()
  local d = branch( true, { branch( true, { leaf( true ), leaf( false ) } ) } )
  eq( SelectionTree.all_checked( d ), false )
end

SelectionTreeSetCheckedSpec = {}

function SelectionTreeSetCheckedSpec:should_toggle_a_leaf_item_on_by_itself()
  local i = leaf( false )

  SelectionTree.set_checked( i, true )

  eq( i.data.checked, true )
  eq( i.data.entry.enabled, true )
end

function SelectionTreeSetCheckedSpec:should_toggle_a_leaf_item_off_by_itself()
  local i = leaf( true )

  SelectionTree.set_checked( i, false )

  eq( i.data.checked, false )
  eq( i.data.entry.enabled, false )
end

function SelectionTreeSetCheckedSpec:should_cascade_to_all_items_when_boss_and_every_item_share_the_same_state()
  local i1, i2 = leaf( false ), leaf( false )
  local b = branch( false, { i1, i2 } )

  SelectionTree.set_checked( b, true )
...
function SelectionTreeSetCheckedSpec:should_cascade_off_to_all_items_when_boss_and_every_item_share_the_same_state()
  local i1, i2 = leaf( true ), leaf( true )
  local b = branch( true, { i1, i2 } )

  SelectionTree.set_checked( b, false )
...
function SelectionTreeSetCheckedSpec:should_write_the_cascaded_state_through_to_each_items_persisted_entry()
  local i1, i2 = leaf( false ), leaf( false )
  local b = branch( false, { i1, i2 } )
...
function SelectionTreeSetCheckedSpec:should_not_cascade_when_at_least_one_item_already_differs_from_the_boss()
...
function SelectionTreeSetCheckedSpec:should_toggle_only_the_boss_itself_when_it_has_no_items()
...
function SelectionTreeSetCheckedSpec:should_cascade_to_every_boss_and_item_when_the_whole_dungeon_subtree_shares_the_same_state()
  ...
  SelectionTree.set_checked( d, true )
...
function SelectionTreeSetCheckedSpec:should_not_cascade_to_any_boss_when_one_boss_already_differs_from_the_dungeon()
...
function SelectionTreeSetCheckedSpec:should_not_cascade_when_a_boss_matches_the_dungeon_but_one_of_its_own_items_does_not()
  local matching_item = leaf( false )
  local differing_item = leaf( true )
  local b = branch( false, { matching_item, differing_item } )
...
SelectionTreeVisibleRowsSpec = {}

function SelectionTreeVisibleRowsSpec:should_only_show_the_dungeon_row_when_collapsed()
  local d = branch( true, { branch( true, { leaf( true ) } ) } )

  local rows = SelectionTree.visible_rows( { d } )
...
function SelectionTreeVisibleRowsSpec:should_show_bosses_once_the_dungeon_is_expanded()
...
function SelectionTreeVisibleRowsSpec:should_show_items_once_the_boss_is_also_expanded()
  local i = leaf( true )
...
function SelectionTreeVisibleRowsSpec:should_desaturate_boss_and_items_when_the_dungeon_is_unchecked()
  local i = leaf( true )
  local b = branch( true, { i } )
  b.data.expanded = true
  local d = branch( false, { b } )
...
function SelectionTreeVisibleRowsSpec:should_desaturate_dungeon_and_boss_when_a_single_item_is_unchecked_but_not_the_item_itself()
...
function SelectionTreeVisibleRowsSpec:should_desaturate_the_direct_parent_boss_of_an_unchecked_leaf()
  local checked_item = leaf( true )
  local unchecked_item = leaf( false )
  local b = branch( true, { checked_item, unchecked_item } )
  b.data.expanded = true
...
function SelectionTreeVisibleRowsSpec:should_desaturate_the_grandparent_dungeon_of_an_unchecked_leaf()
  local checked_item = leaf( true )
  local unchecked_item = leaf( false )
  local b = branch( true, { checked_item, unchecked_item } )
  b.data.expanded = true
...
function SelectionTreeVisibleRowsSpec:should_not_desaturate_a_sibling_boss_unrelated_to_the_unchecked_leaf()
  local unchecked_item = leaf( false )
