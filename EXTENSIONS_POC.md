...
| Black Temple SR limit | Extracted to `RollForBtSrLimitCheck`. The rule is one raid's, so core carried it for every raid it will never apply to. It reaches `/src` by subscribing to `softres_checked`, which `RollForSoftResIt` emits after running its own checks -- neither addon calls into the other |
| Reporting across extensions | An event on the bus, not a cross-addon call. The addon that owns the command says when it has run; anything else with something to report about the same data adds its lines after. Avoids a dependency in the direction that would make the TOC dependency circular |
| `on_enable` | Optional, as `on_ready` already was -- but not both. An extension whose whole job needs a component core has not built yet has no use for the declaration phase; one that does nothing in either phase is a switch that switches nothing |
