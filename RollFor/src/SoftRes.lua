...
local transform = modules.SoftResDataTransformer.transform
...
function M.new( db )
  ...
  local function clear( report )
    if modules.count_elements( softres_data ) == 0 then return end
    softres_data = {}
    persist( nil )
  ...
  local function get( item_id )
    return softres_data[ item_id ] and softres_data[ item_id ].players or {}
  end
