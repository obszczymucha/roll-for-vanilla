...
function M.new( popup, roll_controller, roll_tracker, finish_early, cancel_roll, raid_roll )
  ...
  local function generate_content()
    ...
    if data.status.type == S.Finished and data.status.winner and data.status.winner.roll then
      ...
      table.insert( result, { type = "text", value = string.format( "%s wins the %s roll with a %s.", player, roll_type, roll ), padding = 10 } )
