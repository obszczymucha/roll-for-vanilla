...
function M.new( popup, roll_controller, roll_tracker, config, finish_early, cancel_roll, raid_roll )
  ...
  local function generate_content()
    ...
    if data.status.type == S.Finished and current_iteration and current_iteration.rolling_strategy == RS.RaidRoll then
      local winner = data.status.winner
      local player = c( winner.name, winner.class )
      table.insert( result,
        { type = "text", value = string.format( "%s wins the %s.", player, blue( "raid-roll" ) ), padding = 7 } )
    ...
    if data.status.type == S.Finished and data.status.winner and data.status.winner.roll then
      local winner = data.status.winner
      local player = c( winner.name, winner.class )
      local roll_type = r( winner.roll_type )
      local roll = blue( winner.roll )

      table.insert( result,
        { type = "text", value = string.format( "%s wins the %s roll with %s %s.", player, roll_type, article( winner.roll ), roll ), padding = 10 } )
  ...
  local function refresh()
    popup:refresh( generate_content() )
  end
