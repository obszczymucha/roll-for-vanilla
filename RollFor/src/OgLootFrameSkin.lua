...
function M.new( frame_builder )
  ...
  local function dropped_item( parent )
    ...
    container.quantity = gui.create_text_in_container( "Frame", container.icon, 20, "CENTER", nil, "text" )
    ...
    local function resize()
      ...
      local total_width = icon_width + bind_width + text_width + comment_width
    ...
    local on_width = container.SetWidth
    ...
    local function update_textures( width )
      ...
      middleleft:SetPoint( "TOPLEFT", container, "TOPLEFT", 0, 0 ) -- Just use 0 here
      ...
      middleright:SetPoint( "TOPRIGHT", container, "TOPRIGHT", 0, 0 ) -- And 0 here
    end

    container.SetWidth = function( self, width )
      local wi = width < min_width and min_width or width
      on_width( self, wi )
      update_textures( wi )
    end
  ...
  local function header( on_drag_stop, on_show, on_hide )
    ...
    frame.SetWidth = function( self, width )
      local w = width < min_width and min_width or width
      set_width( self, w )
      update( w )
    end
