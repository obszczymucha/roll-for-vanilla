...
function M.new( announce, ace_timer, group_roster, item, winner_tracker, master_loot_candidates, show_popup, roll_tracker )
  ...
  local function announce_rolling()
    ...
    roll_tracker.start( RollingStrategy.RaidRoll, item )
  ...
  local function on_roll( player, roll, min, max )
    ...
    roll_tracker.stop( { rolling_strategy = RollingStrategy.RaidRoll, player_name = m_winner.name, player_class = m_winner.class } )
