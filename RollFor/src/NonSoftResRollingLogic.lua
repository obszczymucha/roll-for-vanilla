...
function M.new( announce, ace_timer, group_roster, item, count, info, seconds, on_rolling_finished, config, roll_tracker )
  ...
  local function on_roll( player_name, roll, min, max )
    ...
    roll_tracker.add( player_name, player and player.class, roll_type, roll )
  ...
  local function accept_rolls()
    ...
    roll_tracker.start( RollingStrategy.NormalRoll, item, count, info, seconds )
  end
  ...
  return {
    ...
    get_rolling_strategy = function() return modules.Types.RollingStrategy.NormalRoll end
  }
end
